...
def setup_training(self):
    ...
    self.total_step = 0
    # self.model.buffer_capacity = self.buffer_capacity
    self.model.batch_size = self.batch_size
    self.model.use_soft_update = True
    self.model.use_noisy  = self.use_noisy
    self.model.target_update_freq = UPDATE_FREQUENCY
    print("setting up replay_buffer from scartch")
    self.replay_buffer = N_Steps_Prioritized_ReplayBuffer(state_dim= self.state_dim, batch_size=self.batch_size
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if not self.use_noisy:  # Decay epsilon
    ...
    self.rule_prob = self.rule_prob - self.rule_prob_decay if self.rule_prob - self.rule_prob_decay > self.rule_prob_min else self.rule_prob_min


    cur_transition = Transition(s, a, s_, r)._asdict()
    self.replay_buffer.store_transition(s, a,  r, s_, dw, done)
    ...
    if self.t_step == 0:
        if self.replay_buffer.current_size >= self.batch_size:
            ...
            self.logger.info(f'updating when stepping forward, soft update == {self.model.use_soft_update}')
            self.model.update(self.replay_buffer, self.total_step)
...
def end_of_round(self, last_game_state: dict, last_action: str, state_after_last_game_state: dict, events: List[str]):
    ...
    if last_game_state['self'][1] >= self.max_score:
        ...
        with open("max_score_model.pt", "wb") as file:
            pickle.dump(self.model, file)
