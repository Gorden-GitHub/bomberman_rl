...
def act(self, game_state: dict) -> str:
    ...
    self.logger.debug(ACTIONS[action_index])
    self.logger.debug("------------------------------------------------------------------------")
...
def state_to_features(self, game_state: dict):
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    if len(coins) == 0:
        ...
        for cx, cy in coins:
            cfield[cx, cy] = 2
        features.append(BFS(self, (x, y), cfield, 2))
    ...
    for d in directions[-4:]:
        ...
        for xy in others:
            if d == xy:
                ...
                break
    if features[1] != 'met':
        features[1] = BFS(self, (x, y), arena, 1)
    ...
    if bfield[x, y] == -2:
        features[3] = BFS(self, (x, y), bfield, 0, -2, arena)
    for d in directions[-4:]:
    ...
    if features[4] != 'met':
        ...
        if others:
            for xy in others:
                efield[xy[0], xy[1]] = 4
            features[4] = BFS(self, (x, y), efield, 4)
...
def BFS(self, self_pos, field, goal, ignore = 100, hfield = np.array(0)):
