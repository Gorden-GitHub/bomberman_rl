...
class PPO_discrete:
    def __init__(self, state_dim, hidden_width, action_dim = 6, batch_size = 2e12, mini_batch_size = 2e9, max_train_steps = 1000, lr_a = 3e-4, lr_c = 3e-4,
    ...
    def update(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                # a/b=exp(log(a)-log(b))
                ratios = torch.exp(a_logprob_now.detach().cpu() - a_logprob[index])  # shape(mini_batch_size X 1)

                surr1 = ratios * adv[index].detach().cpu()  # Only calculate the gradient of 'a_logprob_now' in ratios
                surr2 = torch.clamp(ratios, 1 - self.epsilon, 1 + self.epsilon) * adv[index].detach().cpu()
                actor_loss = -torch.min(surr1, surr2) - self.entropy_coef * dist_entropy.detach().cpu()  # shape(mini_batch_size X 1)
                # Update actor
